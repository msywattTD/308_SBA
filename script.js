...
function newStudent(submissions) {
  for (i = 0; i < submissions.length; i++) {
    if (!idSet.has(submissions[i].learner_id)) {
      //student ID does not exist yet
  ...
  for (i = 0; i < result.length; i++) {
    ...
    for (j = 0; j < submissions.length; j++) {
      ...
      if (result[i].id == submissions[j].learner_id) {
        //matching student with their assignments
        ...
        for (k = 0; k < ag.assignments.length; k++) {
          if (ag.assignments[k].id == submissions[j].assignment_id) {
            //matches assignment ID to get the due date and max points
            dueDate = ag.assignments[k].due_at;
            const submitDate = submissions[j].submission.submitted_at;
            if (dueDate > today) {
              continue;
            } //skips adding to the runningScore if its not due yet

            try {
              //making sure its a number that's not 0
            ...
            submitDate > dueDate ? (assignScore -= assignMax * 0.1) : null; //late deduction
            runningScore += assignScore;
          }
        }
        if (dueDate > today) {
          continue;
        } //skips adding the assignment to the student object if the assignment isn't due yet
